const events = [
  // arts category id=0
  [
    ["T-SHIRT PAINTING", "./images_arts/6.jpg"],
    ["MANDALA ART", "./images_arts/5.jpg"],
    ["FIND AND PAINT", "./images_arts/1.jpg"],
    ["FREE STYLE", "./images_arts/3.jpg"],
    ["FACE PAINTING", "./images_arts/2.jpg"],
    ["POSTER MAKING", "./images_arts/4.jpg"],
  ],


  // dance category id=1
  [
    ["CLASSICAL / SEMI<br> CLASSICAL & FOLK", "./images_dance/2.jpg"],
    ["WESTERN / <br>FREESTYLE", "./images_dance/4.jpg"],
    ["GROUP DANCE", "./images_dance/1.jpg"],
    ["STREET DANCE", "./images_dance/3.jpg"],
    // ["DRONE DRAG"],
  ],

  // sports category id=2
  [
    ["CRICKET", "./image_sports/1.jpg"],
    ["FUTSAL", "./image_sports/7.jpg"],
    ["CHESS", "./image_sports/2.jpg"],
    ["BADMINTON", "./image_sports/5.jpg"],
    ["VOLLEYBALL", "./image_sports/3.jpg"],
    ["BASKETBALL", "./image_sports/6.jpg"],
    ["TABLE TENNIS", "./image_sports/4.jpg"],
  ],

  // music category id=3
  [
    ["INDIAN SOLO", "./images_music/3.jpg"],
    ["INDIAN DUET", "./images_music/5.jpg"],
    ["WESTERN SOLO", "./images_music/2.jpg"],
    ["WESTERN DUET", "./images_music/6.jpg"],
    ["GROUP SONG", "./images_music/1.jpg"],
    ["BATTLE OF BANDS", "./images_music/4.jpg"]
  ],

  // drama category id=4
  [
    ["MONOACT /<br> DUET ACT", "./images_drama/2.jpg"],
    ["CONVENTIONAL<br>DRAMA", "./images_drama/3.jpg"],
    ["NUKKAD NATAK", "./images_drama/1.jpg"]
  ],

  // literary category id=5

  [
    ["HINDI CREATIVE <br>WRITING", "./images_literary/3.jpg"],
    ["ENGLISH CREATIVE <br>WRITING", "./images_literary/2.jpg"],
    ["DEBATE", "./images_literary/1.jpg"],
    ["POP QUIZ", "./images_literary/4.jpg"],
    ["ANIME QUIZ", "./images_literary/5.jpg"],
    ["JAM", "./images_literary/5.jpg"],
    ["SHARK TANK", "./images_literary/4.jpg"],
    ["OPEN MIC", "./images_literary/6.jpg"],
  ],
  [
    ["FASHION SHOW","./images_fashion/fashion.jpeg"],
  ],
];

const category_img = [
  // arts category id=0
  "./images/arts.svg",

  // dance category id=1
  "./images/dance.svg",

  // sports category id=2
  "./images/sports.svg",

  // music category id=3
  "./images/music.svg",

  // drama category id=4
  "./images/drama.svg",

  // literary category id=5
  "./images/literary.svg",

  // literary category id=6
  "./images/fashion.svg",
];

export { events, category_img };